...
func (suite *TestSuite) TestGetTokenARate() {
	for _, tc := range []struct {
		...
		// - Check bonus being zero
		{
			...
			inTokenDenom:           "uusdt",
			swapFee:                sdk.ZeroDec(),
			expRate:                sdk.MustNewDecFromStr("1.000000000000000000"),
			expErr:                 false,
		},
		...
		// - Check bonus be zero
		{
			...
			expRate:                sdk.MustNewDecFromStr("3.000000000000000000"),
			expErr:                 false,
		},
	} {
		suite.Run(tc.desc, func() {
			...
			rate, _ := pool.GetTokenARate(suite.ctx,
				suite.app.OracleKeeper,
				&pool,
				tc.poolAssets[0].Token.Denom,
				tc.poolAssets[1].Token.Denom,
				suite.app.AccountedPoolKeeper,
			)

			suite.Equal(rate, tc.expRate)
		})
	}
}
